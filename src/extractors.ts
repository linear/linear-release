...
export function extractLinearIssueIdentifiersForCommit(commit: CommitContext): ExtractedIdentifier[] {
  ...
  if (branchDepth % 2 === 1) {
    debug(`Skipping revert branch "${commit.branchName}" (depth ${branchDepth}) for commit ${commit.sha}`);
    return [];
  }
  const { depth: messageDepth } = parseRevertMessage(commit.message ?? "");
  if (messageDepth % 2 === 1) {
    debug(`Skipping revert message (depth ${messageDepth}) for commit ${commit.sha}`);
    return [];
  }
...
export function extractPullRequestNumbersForCommit(commit: CommitContext): number[] {
  ...
  if (/^Revert "/i.test(message)) {
    debug(`Skipping revert commit ${commit.sha} with message: "${message}"`);
    return [];
  }
  ...
  if (getRevertBranchDepth(commit.branchName) % 2 === 1) {
    debug(`Skipping revert merge commit ${commit.sha}`);
    return [];
  }
  ...
  if (squashMatch) {
    debug(`Found PR number ${squashMatch[1]} in commit ${commit.sha} using squash format: "${message}"`);
  ...
  if (mergeMatch) {
    debug(`Found PR number ${mergeMatch[1]} in commit ${commit.sha} using merge format: "${message}"`);
  ...
  if (prNumbers.length === 0) {
    ...
    for (const match of messageMatches) {
      debug(`Found PR number ${match[1]} in commit ${commit.sha} by extracting from message: "${message}"`);
