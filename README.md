...
| `--no-branch-ref-detection` | `sync`                       | Disable issue attribution from Git branch refs; keep commit-message and PR/MR detection                                                                                                                                                                              |
...
By default, `sync` can extract issue identifiers from Git branch refs pointing at regular commits in the scan. Those refs can include unrelated branches created from shipped commits without any new work.

Use `--no-branch-ref-detection` to ignore these refs:

```bash
linear-release sync --no-branch-ref-detection
```

Issue identifiers in commit messages, source branches recorded in merge messages, PR/MR references, and `--issue-pattern` detection remain enabled. The flag applies to both added and reverted issue attribution.

This disables all attribution from branch refs, including legitimate fast-forward merges whose issue key exists only in the branch name. Those workflows need an issue identifier in the commit message or a PR/MR reference that Linear can resolve. Omitting the flag preserves existing behavior.
